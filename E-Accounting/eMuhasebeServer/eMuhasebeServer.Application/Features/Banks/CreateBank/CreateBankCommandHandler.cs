    ...
    ICacheService cacheService) : IRequestHandler<CreateBankCommand, Result<string>>
{
    ...
    {
        bool isIBANExists = await bankRepository.AnyAsync(b=> b.IBAN == request.IBAN,cancellationToken);
        if (isIBANExists)
        {
            return Result<string>.Failure("Bu IBAN Daha Önce Kaydedilmiş");
        }
        ...
        return "Banka Kaydı Başarıyla Oluşturuldu";
    }
}
