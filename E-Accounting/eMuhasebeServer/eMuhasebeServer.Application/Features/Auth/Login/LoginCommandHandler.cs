...
namespace eMuhasebeServer.Application.Features.Auth.Login
{
        ...
        ICacheService cacheService) : IRequestHandler<LoginCommand, Result<LoginCommandResponse>>
    {
        public async Task<Result<LoginCommandResponse>> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            List<CompanyUser> companieUsers = await companyUserRepository
            ...
            if(companieUsers.Count > 0)
            {
                ...
                companies = companieUsers.Select(p=> new Company
                {
                    Id = p.CompanyId,
                    Name = p.Company!.Name,
                    TaxDepartment = p.Company!.TaxDepartment,
                    TaxNumber = p.Company!.TaxNumber,
                    FullAddress = p.Company!.FullAddress
                }).ToList();
            }
